...
# Folder ID where invoices will be uploaded
FOLDER_ID = "1GwKcp0mPEo-PlBHiHthxblTMmMoCUxQo"

def generate_pdf(invoice_number, invoice_date, client_name, client_address, client_phone, df, filename):
    pdf = FPDF()
    pdf.add_page()

    # Logo (optional local path)
...
if st.button("Generate & Upload Invoice"):
    ...
    if valid_df.empty:
        ...
        generate_pdf(invoice_number, invoice_date, client_name, client_address, client_phone, valid_df, filename)
        ...
        st.download_button("📄 Download PDF", data=open(filename, "rb").read(), file_name=filename, mime="application/pdf")
