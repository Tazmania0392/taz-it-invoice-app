...
if st.button("Generate Invoice"):
    ...
    for i, row in item_df.iterrows():
        if pd.isna(row["Description"]):
            continue
        total = row["Qty"] * row["Rate (AWG)"]
        subtotal += total
        values = [row["Description"], row["Units"], row["Qty"], row["Rate (AWG)"], total]
    ...
    total_row = start_row + 2 + len(item_df)
    for i, (label, val) in enumerate([("Subtotal", subtotal), ("Tax (12%)", tax), ("Total", grand_total)]):
        ws.merge_cells(start_row=total_row + i, start_column=1, end_row=total_row + i, end_column=4)
        ws.cell(row=total_row + i, column=1).value = label
        ws.cell(row=total_row + i, column=1).alignment = right
        ws.cell(row=total_row + i, column=1).font = bold
        ws.cell(row=total_row + i, column=1).border = border

        ws.cell(row=total_row + i, column=5).value = val
        ws.cell(row=total_row + i, column=5).alignment = center
        ws.cell(row=total_row + i, column=5).border = border

    ws[f'A{total_row + 3}'] = "Thank you for your business!"
    ws[f'A{total_row + 4}'] = "Payment due within 14 days."
