...
st.write("✅ Google credentials read from secrets")
...
st.write("✅ Google API clients initialized")
...
def generate_pdf(df):
    pdf = FPDF()
    pdf.add_page()
    st.write("📄 PDF page created")

    try:
        pdf.image("tazit_logo_pdf.png", 150, 10, 40)
        st.write("📷 Logo loaded successfully")
    except Exception as e:
        st.error(f"⚠️ Failed to load logo: {e}")
...
def log_to_sheet(date, invoice_no, name, amount, status, link):
    st.write("📝 Logging to Google Sheet...")
